...
ax1.plot(sdt_14_15,c=c1,alpha=alph,lw=lw,label='SDARN')
ax1.fill_between(np.arange(np.shape(sdt_14_15)[0]), 
                    (sdt_14_15 - fit_v_amp * 2),(sdt_14_15 + fit_v_amp * 2),alpha=alpha_l,color=c1)

ax1.plot(sdt_sd - offset_a,c=c2,ls='-',alpha=alph,lw=lw,label='PRISM-SDARN')
ax1.plot(sdt_sd2 - offset_a,c=c3,ls='--',alpha=alph,lw=lw,label='OnlyThermal-SDARN')
ax1.plot(sdt_sd3,c=c4,ls='-.',alpha=alph,lw=lw,label='OnlyLunar-SDARN')
...
ax2.errorbar(np.arange(len(phase_sdt[::fig_sample])),phase_sdt[::fig_sample],yerr=fit_v_phas[::fig_sample] * 2,
          fmt='x',c=c1,ms=ms,label=r'SDARN',capsize=1,elinewidth=1.25,capthick=0,errorevery=1)
ax2.plot(dphase_sdt_v[::fig_sample],
          '.',c=c2,ms=ms,label=r'PRISM-SDARN')
ax2.plot(dphase_sdt_v2[::fig_sample],
          '2',c=c3,ms=ms,label=r'OnlyThermal-SDARN')
ax2.plot(dphase_sdt_v3[::fig_sample],
          '+',c=c4,ms=ms,label=r'OnlyLunar-SDARN')
...
ax2.set_xlabel(r'Days since Dec 1st 2012',fontsize=fs)
ax2.xaxis.set_ticks(xlocs)
ax2.xaxis.set_ticklabels(xlocs,size=ts)
ax2.grid(True)
ax2.set_xlim(xmin / fig_sample,xmax / fig_sample)
ax2.set_ylim(0,2*np.pi)

ax1.legend(ncol=4,loc=2,prop={'size':lss},markerscale=2,bbox_to_anchor=(-0.001, 1.0),framealpha=0.6)
ax2.legend(ncol=4,loc=2,prop={'size':lss},markerscale=2,bbox_to_anchor=(-0.001, 0.2),framealpha=0.6)

fw = 'bold' # text font weight
ax1.text(13.5,25.,'(a)',fontsize=fs + 4,fontweight=fw)
ax2.text(13.5,2 * np.pi + .5,'(b)',fontsize=fs + 4,fontweight=fw)
...
ax2.vlines(ssw_t,0,100,color=ssw_c,ls=ssw_ls,lw=lw_ssw) # x, ymin, ymax
ax1.vlines(ssw_t2,0,100,color=ssw_c,ls=ssw_ls,lw=lw_ssw) # x, ymin, ymax
ax2.vlines(ssw_t2,0,100,color=ssw_c,ls=ssw_ls,lw=lw_ssw) # x, ymin, ymax
ax1.vlines(ssw_t3,0,100,color=ssw_c,ls=ssw_ls,lw=lw_ssw) # x, ymin, ymax
ax2.vlines(ssw_t3,0,100,color=ssw_c,ls=ssw_ls,lw=lw_ssw) # x, ymin, ymax

save_f = '/home/wim/Desktop/Projects/iSSW/paper/figures/'
# plt.savefig(save_f + 'superdarn_model_comp.png',dpi=300, bbox_inches="tight")

save_f = '/home/wim/Desktop/Thesis/figures/png/'
# plt.savefig(save_f + 'sdarn.png',dpi=300, bbox_inches="tight")
