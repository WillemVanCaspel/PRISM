C=======================================================================
      SUBROUTINE PY_HEATWAC(DPGQSP,TIM, WATERCOEFF, OZONECOEFF, MESOCOEFF, TROPINDEX, STRATINDEX)
C=======================================================================
C     Assimilate Python-generated diabatic heating fields [K s-1]
C     represented by spherical harmonic coeffs on sigprim model lvls.
...
C-----------------------------------------------------------------------
      ...
      IF (FIRST) THEN
        OPEN(WLUN, FILE=TRIM(INFOLD)//TRIM(TTIDEFILE), STATUS='OLD',SHARED,
...
C     interpolate to current time and cycle periodically over data time range
     ...
     1    (((READN0(I,J,L), I=1, MAXM), J=1, MAXN), L=1, MAXL)
     ...
     1    (((READN1(I,J,L), I=1, MAXM), J=1, MAXN), L=1, MAXL)
...
C     Linearly interpolate in time
     ...
     1          + MESOCOEFF*WT0*HEATIN0(M,N,K) + MESOCOEFF*WT1*HEATIN1(M,N,K))
            ELSE IF (L .LE. TROPINDEX) THEN
              DPGQSP(M,N,JPOT,L)=(DPGQSP(M,N,JPOT,L)
     1          + OZONECOEFF*WT0*HEATIN0(M,N,K) + OZONECOEFF*WT1*HEATIN1(M,N,K))
            ELSE
              DPGQSP(M,N,JPOT,L)=(DPGQSP(M,N,JPOT,L)
     1          + WATERCOEFF*WT0*HEATIN0(M,N,K) + WATERCOEFF*WT1*HEATIN1(M,N,K))
            ENDIF
	      END DO
        END DO
      END DO
