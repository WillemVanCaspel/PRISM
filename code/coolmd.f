...
C+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
C    QIRMID,   RADCO2,   DETIJ,    INVERT,   AVT21,    AVT32,    
C    SPT,      RADO3,    DETIJR,   RADH2O,   INPUTC,   NLTEC,    
C
C  To calculate the IR radiative cooling rate for middle atmosphere modeling (8/95)


CC ==== IR radiative cooling by CO2, H2O, and O3 in the middle atmosphere ==== 

      SUBROUTINE QIRMID(QIR,TEM,CO2,O3,H2O,OXY,IM,KLBD)
C  To calculate the IR cooling rate in the middle atmosphere (K day^-1)
...
CC  To invert the matrix D,  M=N+1. Mth row can be any values. REAL*8
      IMPLICIT REAL*8 (A-H,O-Z)
      DIMENSION D(N,M)
      KK=0
      JJ=0
  ...
  10  CONTINUE
      ...
      IF(TEM(I).GE.200.0D0) VT1M=2.5D-21*(1.0D0
     &                          +0.03D0*(TEM(I)-200.D0))
...
c  KLBD=lower boundary grid, KLBD=1 => surface as a lower boundary.
...
CC  To calculate the DELTA(i,j) used for mixing ratio interpolations (ZHU, 1993)
      IMPLICIT REAL*8 (A-H,O-Z)
...
C  To calculate the H2O cool-to-space cooling rate.  !  ZM(KM)>ZM(1)
      IMPLICIT REAL*8 (A-H,O-Z)
      PARAMETER (KM=76)     !!! specified by users:  KM=IM,   ZM(KM)>ZM(1)
      COMMON /CTXZ23/ Z0(KM),P0(KM),T0(KM),GDP(KM)
...
C  The output nonLTE curtis matrix is stored in EE.        !  ZM(KM)>ZM(1)
